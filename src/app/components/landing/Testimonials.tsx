      ...
      {[...new Array(2)].fill(0).map((_, index) => (
        <React.Fragment key={index}>
          {props.testimonials.map(({ text, imageSrc, name, username }, i) => (
            <div
              className="p-10 border border-light-gray rounded-3xl text-[14px] shadow-[0_7px_14px_#EAEAEA] max-w-xs w-full"
              key={i}
            >
              <div>{text}</div>
...
export const Testimonials = () => {
  return (
    <section className="bg-white py-[72px] md:w-[1000px] flex justify-center sm:py-24 mx-auto md:-mt-[0px] mt-[500px]">
      <div className="">
        <div className="flex flex-col justify-center items-center md:w-full w-[400px]">
          <h2 className="text-center md:text-5xl text-3xl font-bold tracking-tighter text-black">
            What our <span className="text-violet">users say</span>
          </h2>
          <p className="md:text-xl text-md text-center mt-5 md:w-[600px] w-full px-[20px] text-dark-gray">
